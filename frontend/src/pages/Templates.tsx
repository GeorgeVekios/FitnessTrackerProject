    ...
    <>
      ...
      <div className="min-h-screen bg-slate-950">
        <div className="container mx-auto px-4 py-6 max-w-6xl">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-slate-100 mb-2">Workout Templates</h1>
              ...
              {templates.map((template) => (
                <div key={template.id} className="card p-6">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4">
                    <div>
                      <h5 className="text-xl font-semibold text-slate-100 mb-1">
                        {template.name}
                      </h5>
                      {template.description && (
                        <p className="text-sm text-slate-400 italic">{template.description}</p>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <button
                        onClick={() => handleUseTemplate(template.id)}
                        className="btn-success flex-1 sm:flex-none inline-flex items-center justify-center gap-2"
                      >
                  ...
                  <div>
                    <strong className="block text-slate-300 mb-2">Exercises:</strong>
                    <ul className="space-y-2">
                      {template.exercises.map((ex) => (
                        <li
                          key={ex.id}
                          className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-slate-800/50 rounded-lg p-3 gap-2"
                        >
                          <div>
